   /* This program creates a diamond of a given character and height.      */ 
...
public class DiamondProg
{
   ...
   private static String pyramidLine(int lineNumber)
   {
      ...
      line += spacesForUpsidePyramidLine(lineNumber);
      line += symbolsForUpsidePyramidLine(lineNumber);
      line += "\n";
      return line;
   }
   ...
   //-----------------------------------------------------------------
   //  Returns a string of spaces for the given line of the upsideDownPyramid.
   //-----------------------------------------------------------------
   private static String spacesForUpsidePyramidLine(int lineNum)
   {
      String lineSpaces = "";
      for (int spacesCount = MARGIN + 1; spacesCount <= (MARGIN + height + 1 - lineNum); spacesCount++)
      {
         lineSpaces += " ";
      }
      return lineSpaces;
   ...
   //-----------------------------------------------------------------
   //  Returns a string of symbols for the given line of the upsideDownPyramid.
   //-----------------------------------------------------------------
   private static String symbolsForUpsidePyramidLine(int lineNum)
   {
      String lineSymbols = "";
      for (int symbolsCount = ((lineNum * 2) - 1); symbolsCount >= 1; symbolsCount--)
      {
         lineSymbols += brickCharacter;
      }
      return lineSymbols;
